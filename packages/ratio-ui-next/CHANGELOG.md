...
- 3b1c219: Ship the package's licence. The MPL-2.0 `LICENSE` file, the `license` and
  `description` fields, and the SPDX headers on the sources all landed after the
  1.0.0 tag was cut, so no published version has ever carried them — npm still
  serves 0.2.0, which has neither a licence nor a description. This is also the
  package's first release through the automated pipeline, so it is the first to
  be published with provenance.

## 1.0.0

### Patch Changes

- Updated dependencies [1ed318e]
- Updated dependencies [d6a16a9]
- Updated dependencies [c104ddf]
- Updated dependencies [feb08fa]
- Updated dependencies [0a5b7b4]
- Updated dependencies [ad7d4d2]
- Updated dependencies [be1774a]
...
- b58f89d: Make `@eventuras/ratio-ui` a peer dependency (was a regular dependency). Consumers now provide a single shared copy of ratio-ui, avoiding duplicate-instance issues (e.g. mismatched React context). Install `@eventuras/ratio-ui` (>=2.7.0) alongside this package.
