...
import { mergeSanitizeSchemas } from '@eventuras/markdown-react';
...
describe('MarkdownContent', () => {
  ...
  describe('remarkCallout', () => {
    ...
    const setup = async () => {
      const { remarkCallout } = await import('@eventuras/markdown-react');
      const { calloutComponents } = await import('./calloutComponents');
      const { calloutSanitizeSchema } = await import('@eventuras/markdown-react');
  ...
  describe('sanitize schema merging', () => {
    it('keeps the attributes of every schema naming the same tag', () => {
    ...
    it('keeps default attributes when an extension names the same tag', () => {
      // Replacing instead of merging here would drop `href` from the allowlist
      // and quietly break every link on the page.
      const { container } = render(
        <MarkdownContent
          markdown="[Home](/home)"
