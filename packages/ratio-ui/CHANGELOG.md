...
- 0957542: Three small additions to core, all beta:
...
- 484ef60: `Panel` becomes composable. Form is split from surface: `accent` (`none`,
...
- cbdd2fd: `SectionNav` takes `glass` and `size`. `glass` is the page surface at 88% with
  a backdrop blur, the treatment `Navbar` already has — a detail page stacks the
  two rows, and with one opaque and one frosted they read as different
  materials. `size="sm"` trims the row from 3rem to 2.5rem; a pinned row is
  viewport the reader never gets back, and the labels are the same size either
  way, only the air around them changes. Both default to today's behaviour.

### Patch Changes

- cbdd2fd: `Menu` no longer drags the sticky header out of the viewport. Its popover was
...
- 991f889: New built-in `ink` theme — quiet, near-black, elegant: the palette a site
  ...
  restrained ochre accent; status colours stay semantic. The primary _scale_ is a
...
- 76efc1b: Occasions in `Hero`, completing the set opened in `Navbar`. `Hero.Motif` is
...
- d95a0dc: Occasions in `Navbar` — the named openings an app can fill on a marked day
...
- 76724f7: Fix `NavTree` rows without an `href` rendering as `<a href="#">`.
  ...
  Both now render a `<span>` with the same row chrome and `cursor-default`, matching what the icon rail already did. Vertical _branches_ without `href` are unchanged — they were already a `<button aria-expanded>` toggle, which is the right affordance.
...
- e86f86b: Fix `Navbar.Brand`, `Tabs.Item` and `Button.Avatar`/`Button.Label` being `undefined` in React Server Components.

  `Navbar`, `Tabs` and `Button` are `'use client'` modules that attached their compound statics inside that same module. Across the RSC boundary a server component sees a client module's exports as _references_, and a property lookup on a reference yields `undefined` — so `<Navbar>` rendered fine while `<Navbar.Brand>` threw:
...
- e26daa3: Fix `errors` on the Input family rejecting react-hook-form's `FieldErrors`.
...
- 95558d3: Type the Input family's props: `InputProps`/`InputFieldProps` now extend
...
- b225ffa: Two weeks of delivery, rolled into one minor release.

  **New — `Tree` (beta):** an accessible, sortable hierarchy at `@eventuras/ratio-ui/tree` (new `./tree` entrypoint). Built on React Aria's `Tree`: expand/collapse and keyboard navigation, single/multiple selection with optional checkboxes, and opt-in drag-to-reorder / drop-to-reparent (pointer _and_ keyboard). Expansion and selection are controllable (`expandedKeys`/`onExpandedChange`, `selectedKeys`/`onSelectionChange`), rows render via `renderNode(node, state)`, and a headless `useSortableTree` hook exposes the state + drag-and-drop wiring for custom shells. It's the interactive sibling to `TreeView` (navigation) and `DataTree` (read-only data). Also adds the `GripVertical`, `Folder`, `FolderOpen`, and `Tag` icons.
